...
def getTimes(date, lat, lng, height=0):
    ...
    result = dict(
        solarNoon=fromJulian(Jnoon).strftime('%H.%M'),
        nadir=fromJulian(Jnoon - 0.5).strftime('%H.%M')
    )
    ...
    for i in range(0, len(times)):
        ...
        result[time[1]] = fromJulian(Jrise).strftime('%H.%M')
        result[time[2]] = fromJulian(Jset).strftime('%H.%M')
...
def getMoonTimes(date, lat, lng):
    ...
    if (rise):
        result["rise"] = hoursLater(t, rise).strftime('%H.%M')
    if (sett):
        result["set"] = hoursLater(t, sett).strftime('%H.%M')
